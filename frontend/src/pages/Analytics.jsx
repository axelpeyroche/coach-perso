...
export default function Analytics({ dark }) {
  ...
  const rpeData = recup?.tendance_rpe?.map((r) => ({ date: r.date.slice(5), fullDate: r.date, reel: r.rpe_reel, cible: r.rpe_cible })) ?? [];
          ...
          <div className="w-full overflow-x-hidden">
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={rpeData}>
                ...
                <Tooltip formatter={(v, name) => [v != null ? v.toFixed(1) : "—", name]} labelFormatter={(_, payload) => payload?.[0]?.payload?.fullDate ? fmtDate(payload[0].payload.fullDate) : ""} contentStyle={ttStyle} labelStyle={ttLabelStyle} />
                <Legend />
